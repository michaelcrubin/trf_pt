...
make_exp_mc_plots <- function(theta = c(0.01, 0.1, 0.5), k = c(1, 3, 9, 27, 100), n_sims = 50000){
  
  p <- tidyr::expand_grid(theta = theta, k = k, n_sims = n_sims) %>%
    purrr::pmap(., .f = run_exp_sim) %>%
    wrap_plots(ncol = 2) +
    plot_annotation(
      title = paste("Mean k Exp(θ = ",paste(theta, collapse = ", "),")"),
      theme = theme(plot.title = element_text(hjust = 0.5))
    )
  return(p)
}

make_gamma_mc_plots <- function(theta = c(0.01, 0.5),  alpha = c(1.3, 1.8), k = c(1, 3, 9, 27, 100), n_sims = 50000){
  
  p <- tidyr::expand_grid(theta = theta, alpha = alpha, k = k, n_sims = n_sims) %>%
    purrr::pmap(., .f = run_gamma_sim) %>%
    wrap_plots(ncol = 2) +
    plot_annotation(
      title = paste("Mean k Gamma(α=",paste(alpha, collapse=","),"θ = ",paste(theta, collapse = ","),")"),
      theme = theme(plot.title = element_text(hjust = 0.5))
    )
  return(p)
}
